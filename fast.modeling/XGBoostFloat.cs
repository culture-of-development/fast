...
{
    public sealed class XGBoostFloat
    {
        private DecisionTreeFloat[] trees;
        
        public XGBoostFloat(DecisionTreeFloat[] trees)
        {
            this.trees = trees;
        }
        ...
        public static XGBoostFloat Create(string allTrees)
        {
            var treeStrings = treeSplit.Split(allTrees);
            var trees = treeStrings
                .Where(m => !string.IsNullOrWhiteSpace(m))
                .Select(DecisionTreeFloat.Create)
                .ToArray();
            var model = new XGBoostFloat(trees);
            return model;
        }
    }
}