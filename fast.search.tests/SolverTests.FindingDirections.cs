...
namespace fast.search.tests
{
    partial class SolverTests
    {
        ...
        public void TestFindingDirectionsAStarSearchLimaLandmarksHeuristic()
        {
            ...
            var landmarks = new FindingDirectionsState[4];
            landmarks[0] = nodeLocator.GetNearestNeighbours(new[] { -11.9997645637738,-77.08525616778775 }, 1)[0].Value;
            landmarks[1] = nodeLocator.GetNearestNeighbours(new[] { -12.073047431238026,-77.16825444354458 }, 1)[0].Value;
            landmarks[2] = nodeLocator.GetNearestNeighbours(new[] { -12.190050763534796,-76.97454096014161 }, 1)[0].Value;
            landmarks[3] = nodeLocator.GetNearestNeighbours(new[] { -12.0160677109035,-76.87767227591303 }, 1)[0].Value;
            output.WriteLine("Landmarks selected: " + landmarks.Length);
